...
const KIOSK_MODE = false; // ✅ DISABLED - testing mode (will be re-enabled later)
let isKioskLocked = false; // System starts unlocked
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    width: width,
    height: height,
    frame: true,                             // ✅ Show window frame for testing
    fullscreen: false,                       // ✅ Normal window mode
    alwaysOnTop: false,
    skipTaskbar: false,                      // ✅ Show in taskbar
    kiosk: false,                            // ✅ No kiosk mode for testing
  ...
  mainWindow.once('ready-to-show', () => {
    mainWindow.show();
    if (KIOSK_MODE) {
      mainWindow.setFullScreen(true);        // Force fullscreen only in kiosk mode
    }
    mainWindow.focus();
    ...
    if (KIOSK_MODE) {
      ...
      console.log('✅ TESTING MODE - Shortcuts available');
    }
  });
